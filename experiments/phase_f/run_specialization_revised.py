"""Phase 6 REVISED (RNG fix): specialization experiment rerun after fixing the per-robot
RNG construction in src/environment/simulator.py (previously `base_draw + robot_id`,
now independent SeedSequence.spawn() children -- see simulator.py comment and Phase 6/7
"Revised (RNG fix)" report sections). Uses a FRESH seed pool (21000-21005), not the
original 16000-16005 pool, per this task's instructions. Otherwise identical methodology
to the original Phase 6 script (same DelegationPolicy defaults, same C3 local-memory
condition, same activity/entropy/Gini/persistence analysis).

Long-horizon (decisions_per_robot large),
...
SEEDS = list(range(21000, 21006))  # fresh pool, 6 seeds, disjoint from original 16000-16005
LAMBDA_LATENCY = 0.15
N_ROBOTS = 10
...
def run_episode(seed: int, robot_id_map=None, shuffle_order_seed=None, tie_break="first",
    ...
    # alternate tie-breaking control (Section 20): DelegationPolicy.decide() iterates
    # peer_ids in a fixed ascending order and keeps the FIRST candidate on an exact EU
    # tie (strict `>` comparison). Untried peers all carry identical uninformative-prior
    # stats, so early in an episode (and whenever a robot meets a still-untried peer)
    # this is a genuine, frequent exact tie, not a rare floating-point coincidence -- and
    # the fixed ascending iteration order means ties are always won by the lowest
    # untried robot ID. This control randomizes peer evaluation order per decision so
    # ties are broken uniformly at random instead of by ID order.
    tie_break_rng = np.random.default_rng(tie_break_rng_seed) if tie_break_rng_seed is not None else None
...
def run():
    ...
    for seed in SEEDS:
        ...
        np.save(os.path.join(results_dir, f"activity_seed{seed}_revised.npy"), activity)
        all_analysis[str(seed)] = analyze(activity)

    with open(os.path.join(results_dir, "specialization_summary_revised.json"), "w") as f:
    ...
    with open(os.path.join(results_dir, "cross_seed_summary_revised.json"), "w") as f:
