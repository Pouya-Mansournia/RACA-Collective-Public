"""Phase 3: train B3 (learned outcome-aware router) on the counterfactual
dataset, compare B0/B1/B2/B3/Oracle on held-out seeds with regret vs Oracle
and CIs, then run the mandatory distribution-shift test.

Seed pools (disjoint from each other and from Phase 1/2 pools):
  development seeds : 3000-3009 (10)  -- fit the logistic regression
  calibration seeds : 3100-3109 (10)  -- choose escalation threshold tau
  validation seeds  : 3200-3209 (10)  -- sanity check before touching test
  held-out test     : 3300-3319 (20)  -- touched exactly once, for final numbers

Usage: python3 -m experiments.phase_c.run_learned_router
"""
...
DEV_SEEDS = list(range(3000, 3010))
CAL_SEEDS = list(range(3100, 3110))
VAL_SEEDS = list(range(3200, 3210))
TEST_SEEDS = list(range(3300, 3320))
LAMBDA_LATENCY = 0.15
