"""Phase 5 revised: shared experience / cognitive memory, using the CALIBRATED
DelegationPolicy from experiments/phase_d/calibrate_delegation.py instead of the original
hand-picked parameters. Fresh held-out seed pool, disjoint from every prior pool.

Usage: python -m experiments.phase_e.run_memory_revised
"""
...
SEEDS = list(range(20000, 20015))  # fresh pool, disjoint from 14000s/15000s/18000s/19000s
LAMBDA_LATENCY = 0.15
...
def run_episode(seed: int, cond: MemoryConditionConfig, policy: DelegationPolicy) -> dict:
...
def run():
    ...
    for seed in SEEDS:
        for name, cond in CONDITIONS.items():
            all_results[name].append(run_episode(seed, cond, policy))

    with open(os.path.join(results_dir, "raw_episodes_revised.json"), "w") as f:
        json.dump(all_results, f, indent=2)

    summary = {"calibrated_params": params}
